...
# first scrape current weather


SCRAPEDIRECTORY="../Data/json/weather/"
LOCATION="Gent"
...
WEATHERFILENAME="weather-${DATE}_${TIME}"

URL="https://weatherapi-com.p.rapidapi.com/history.json?q=$LOCATION&dt=$DATE&lang=en&hour=$TIME"

# Log the start of the script
echo "Script started at $(date)" >> "${SCRAPEDIRECTORY}/log.txt"

# Log the URL being used
echo "Fetching weather data from: $URL" >> "${SCRAPEDIRECTORY}/log.txt"

# Perform the API request and save the response
curl --request GET \
	--url "{$URL}" \
	--header 'X-RapidAPI-Host: weatherapi-com.p.rapidapi.com' \
	--header 'X-RapidAPI-Key: 591e17f733msh595afbbfe142928p1c8b1cjsna6b834ee0290' >> "${SCRAPEDIRECTORY}/${WEATHERFILENAME}.json"

# Log the completion of the script
echo "Script completed at $(date)" >> "${SCRAPEDIRECTORY}/logs/${DATE}log.txt"



# formatting the json file to csv

# Directory containing the JSON files
JSONDIRECTORY="../Data/json/weather"
...
for file in "$JSONDIRECTORY"/*.json; do
